...
export class SaleRecipeDialogComponent implements OnInit {
  private fb = inject(FormBuilder);
  private dialogRef = inject(MatDialogRef<SaleRecipeDialogComponent>);
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: { mode: string, type: string, data?: any }) {
    ...
    if (this.mode === 'create') {
      this.title.set('Crear Salida');
    } else if (this.mode === 'edit') {
      this.title.set('Editar Salida');
    } else {
